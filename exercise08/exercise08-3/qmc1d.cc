...
int main(int argc, char ** argv)
{
	...
	// The object is declared in costants.h.
    int seed[4];
    int p1, p2;
	...
	// End of rng initialisasion.

	if(argc != 2)
	{
		cerr << "Please enter the input file name." << std::endl;
		return 1;
	}
    readInput(argv[1]);  
...
{
	totalTranslations++;
	double delta = rng.Rannyu(-delta_translation,delta_translation);
	...
	if(rng.Rannyu()<acceptance_probability)
	{
		for(int i=0;i<timeslices;i++)
			positions[i]+=delta;
...
void brownianBridge()
{
	...
	int starting_point = (int)(rng.Rannyu()*available_starting_points);
	...
	for(int i=0;i<brownianBridgeReconstructions;i++)
	{
		...
		double newcoordinate = rng.Gauss(average_position,sqrt(variance));
...
void brownianMotion(int which) // BM is called only for PIGS simulations
{
	int starting_point, endpoint, left_reco;
		...
		starting_coord = rng.Gauss(average_position,sqrt(variance));
		...
		starting_coord = positions[starting_point];
                ...
                variance = 2*lambda*dtau*(brownianMotionReconstructions+1);
		ending_coord = rng.Gauss(average_position,sqrt(variance));
